...
class SearchController extends Controller
{
    ...
    public function list (Request $request)
    {
        ...
        $power_size = $request->power_size ?? "";

        $cars = Car::select('cars.*')->with(['brand:id,name', 'model:id,name', 'dealer:id,username,fullname,email', 'dealer.detail:user_id,phone,address', 'images:id,car_id,thumbnail,full'])
        ...
        ->when($featured_only == 1, function ($query) use ($brand_id, $model_id)  {
            $query->whereHas('featured', function ($q) use ($brand_id, $model_id) {
